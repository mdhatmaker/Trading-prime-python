...
def calc_autocorrelations(column_name, count, significant):
    ...
    for (index, row) in df.iterrows():
        ...
        df.set_value(index, 'autocorr_1', round(s.autocorr(lag=1),2))
        df.set_value(index, 'autocorr_2', round(s.autocorr(lag=2),2))
        df.set_value(index, 'autocorr_3', round(s.autocorr(lag=3),2))
        df.set_value(index, 'autocorr_4', round(s.autocorr(lag=4),2))
        df.set_value(index, 'autocorr_5', round(s.autocorr(lag=5),2))
